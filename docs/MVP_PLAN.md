...
- 采用关联出牌：`tableReference` 为空时可用任意实体手牌建立首张基准；非空时，主行动实体出牌必须与当前基准牌关联。
- 原计划保留 70 张固定测试卡池；纠正“实验台起火”角色技能身份后，当前普通实体卡池为 68 张，是否补回 70 张待裁定。
...
MVP 0 使用固定测试卡池，不做卡组构筑。移除误入普通卡池的 2 张“实验台起火”后，当前普通实体卡池为 68 张；是否补回到 70 张需要另行裁定，不能用虚构化学牌静默填充。
