...
#### A. Phase 17C（Pre-rename 内部身份准备阶段）活动文件
...
- *明确保护*：Phase 17C **严禁**修改 `src/app/projectRepository.tsx`、`src/app/projectRepository.test.tsx`、`e2e/production/reaction-field.spec.ts`、`e2e/tests/debug-alpha.spec.ts`、`README.md`、`README.zh-CN.md` 中的 live 仓库 URL 与 live Pages URL，必须继续指向 `Chemistry-online-Card-Game`。
...
- `README.md` & `README.zh-CN.md`：更新仓库链接为生效的新仓库 URL；公开 Pages 试玩入口严格保持当前真实可用地址 `https://9947447-alt.github.io/Chemistry-online-Card-Game/`，直至新 Pages 部署与公网验收完成。
...
整体实施时序必须严格遵循：
`Phase 17C (Pre-rename 内部身份准备)` → `Phase 17D (GitHub 仓库改名)` → `Post-rename (代码库链接正式切换 Cutover)` → `验证 / Commit / Push / CI / Merge` → `新版本 Tag / Pages 部署` → `Phase 17E (真实公网验收)`

```
┌─────────────────────────────────────────────────────────────┐
│ Phase 17C: Pre-rename 内部身份准备                          │
...
### 4.1 Phase 17C：代码库内 Pre-rename 内部身份准备
- **执行范围**：
  1. 修改 `package.json` 中的 `"name"` 为 `"reaction-field"`。
  2. 修改 `docs/MVP_PLAN.md` 中对应的目标 Pages/仓库 URL 规划与 Phase 17 路线图记录。
  3. 修改 `docs/CODEX_CAPABILITIES.md` 增补审计快照。
- **Live Link 保护与禁止范围**：
  1. **严禁提前修改 live 仓库链接**：`src/app/projectRepository.tsx`、`src/app/projectRepository.test.tsx`、`e2e/production/reaction-field.spec.ts`、`e2e/tests/debug-alpha.spec.ts`、`README.md`、`README.zh-CN.md` 中的 live 仓库链接必须继续保留当前真实生效地址 `https://github.com/9947447-alt/Chemistry-online-Card-Game`，严禁在仓库改名完成前提前修改为新 slug。
...
- **停止条件与备用 Slug 回退流程（Fallback Procedure）**：
  ...
  - **Step 5（重新完成本地验证与合并边界）**：重新完成本地全量验证（阶段 A 构建、测试、diff 核对）→ 本地 commit → 阶段 B clean 验证 → 取得 push 授权 → push → CI 门禁通过 → PR review & merge 到集成分支。
...
- **执行范围**：
  ...
  4. 修改 `README.md` 与 `README.zh-CN.md` 中的仓库外链（公开 Pages 试玩入口继续保留 live 地址 `https://9947447-alt.github.io/Chemistry-online-Card-Game/` 直至 Pages 部署与公网验收完成）。
  5. 修改 `docs/MVP_PLAN.md` 中对应仓库 URL 记录为已生效。
- **代码库正常修改与合并流程**：
  必须经过标准工作流：只读基线核对 → 白名单文件修改 → 本地全量验证（阶段 A 构建、测试、E2E、diff 核对）→ 本地 commit → 阶段 B clean 验证 → 取得 push 授权 → push → CI 门禁通过 → PR review & merge 到集成分支。
...
- **Pages 重新部署与触发授权**：
  ...
  - 若需要创建新标签触发部署，必须使用新的递增版本和新的不可变标签，严禁移动、覆盖或重新推送 `web-playtest-v0.16.0-alpha.1` 等历史标签。
...
- **代码与链接回滚（Code & Identity Rollback）**：
  - 仓库改名回滚不会自动撤销代码变更。若此前已合并了 post-rename link cutover 或 Phase 17C 代码，必须通过独立代码回滚边界（`git revert` 或经审阅的回滚 commit）恢复 `src/app/projectRepository.tsx`、测试与 `package.json` 中的相应身份；
  - 严禁 hard reset，严禁历史重写；
  - 必须重新完成本地构建与测试验证、PR review 和 merge 流程。
- **发布与 Tag 红线警告**：
